 ...
 * - This file implements a data-oriented ray tracer based on Bidirectional Path
 * Tracing (BDPT).
 * - Component-System Architecture (ECS): Instead of recursive function calls
 * which bloat the stack and cause thread starvation, ray tracing is implemented
 * as a state machine. Paths are explicitly constructed in memory
 * (ComponentCameraPath, ComponentLightPath) and advanced via systems
 * (SystemGenerateRays, SystemIntersectRays, SystemEvaluateLambert, etc.).
 * - Materials:
 *   - Lambert: Perfect diffuse scattering (cosine weighted).
 *   - Metal: Imperfect mirror reflection.
 *   - Light: Emissive surfaces (Sun, Moon).
 *   - Translucent: Schlick's approximation for dielectric reflection/refraction
 * with Beer's Law volumetric absorption.
 * - Scene Geometry & Acceleration:
 *   - Primitives: Sphere, Box (AABB).
 *   - BVH (Bounding Volume Hierarchy): A SAH (Surface Area Heuristic) optimized
 * BVH tree is used to accelerate ray intersection tests from O(N) to O(log N).
 *     The optimize_bvh method allows dynamic runtime rebuilds if objects move
 * or are added.
 ...
 * - Direct light samples bypass the MIS (Multiple Importance Sampling) and go
 * straight to the direct buffer for crisp, noise-free highlights.
 ...
 * - Initial versions used standard recursive ray tracing (experimental ray
 * tracing test).
 * - Refactored to a DOD (Data-Oriented Design) system using task graphs to
 * maximize CPU occupancy (shio: data parallelism, shio: handle recursive ray
 * queries).
 * - Transitioned from Unidirectional Path Tracing to Bidirectional Path Tracing
 * (BDPT) to correctly resolve complex caustics and light transport (basic
 * bidirectional path tracing).
 * - Introduced Translucent material type with subsurface scattering (SSS)
 * approximations.
 * - Switched to Sparse Rendering (sparse rendering to maintain fps) dynamically
 * adjusting the ray_budget depending on frame time to ensure a fluid user
 * experience even during heavy movement.
 * - Applied frequency-domain spatial filtering to suppress variance
 * spikes/fireflies.
 * - Integrated celestial bodies (Sun, Moon) with physically-based atmospheric
 * scattering (Rayleigh/Mie) acting as volumetric light filters rather than
 * arbitrary colored spheres.
 * - Procedural "Minecraft-style" terrain generation created complex
 * multi-primitive environments necessitating the implementation of a BVH
 * (Bounding Volume Hierarchy) (add scene bvh, Add bvh optimization).
 * =============================================================================
 */
#pragma once
...
struct SamplerPCG32
{
    ...
    uint32_t next_u32()
    {
        ...
        state             = oldstate * 6'364'136'223'846'793'005ULL + inc;
        uint32_t xorshifted =
...
struct ONB
{
    ...
    {
        w          = n.normalize();
        Vector3f a = (std::abs(w.x) > 0.9f) ? Vector3f { 0, 1, 0 }
                                            : Vector3f { 1, 0, 0 };
...
inline Vector3f random_in_unit_sphere(SamplerPCG32 & rng)
{
    ...
    // using purely uncorrelated PCG32 to avoid multi-dimensional structural
    // artifacts. Instead of rejection sampling (which has non-deterministic
    // loop times and branch mispredictions), we use spherical coordinates and a
    // cubic root scale to guarantee O(1) execution.
...
struct HitRecord
{
    ...
    inline void set_face_normal(
        const Vector3f & r_dir, const Normal3f & outward_normal)
    {
...
struct Sphere
{
    ...
    void sample_surface(
        SamplerPCG32 & rng, Vector3f & p, Normal3f & n, float & pdf) const
    {
...
struct Box
{
        ...
        const Vector3f & o, const Vector3f & d, float t_min, float t_max) const
    {
        ...
        // Shio: Now correctly returns the exit hit (t1) if the ray starts
        // inside (t0 < t_min)!
    ...
    Normal3f get_normal(const Vector3f & p) const
    {
        ...
        Vector3f n       = {
            local_p.x / extents.x, local_p.y / extents.y, local_p.z / extents.z
        };
...
struct AABB
{
    ...
    bool intersect(const Vector3f & o,
        const Vector3f &            inv_d,
        float                       t_min,
        float                       t_max) const
    {
        float t0 = -1e30f;
...
{
    int pass =
        0; // 0: InitCam, 1: BounceCam, 2: InitLight, 3: BounceLight, 4: Connect

    // Shio: Boot extremely fast with exactly num_cores proportion of rays (e.g.
    // 1/8th of screen on 32-core)
    int ray_budget =
        (800 * 600) / std::max<int>(1, std::thread::hardware_concurrency() / 4);

    std::vector<uint32_t> active_pixels;

    // Fast inline PRNG for unbiased random pixel selection during sparse
    // rendering
    uint32_t xorshift_state = 1'337;
...
struct ServiceCamera
{
    ...
    Vector3f right() const { return { std::cos(yaw), 0.0f, -std::sin(yaw) }; }

    Vector3f up() const { return forward().cross(right()).normalize(); }
};
...
struct ServiceFilm
{
    struct Pixel
    {
        std::atomic<double> direct_r { 0 }, direct_g { 0 }, direct_b { 0 },
            direct_w { 0 };
        std::atomic<double> indirect_r { 0 }, indirect_g { 0 },
            indirect_b { 0 }, indirect_w { 0 };
    };

    std::vector<Pixel> pixels;
    std::vector<Pixel>
        expired_pixels; // Double-buffering state to smoothly decay motion-blur
    ...
    float expired_decay_scalar =
        1.25f; // User tunable: 25% faster decay for expired ghost buffer
    ...
    float evaluate_filter(float x, float y) const
    {
        auto mitchell_1d = [this](float v) {
            ...
            {
                return (1.0f / 6.0f) *
                    ((12.0f - 9.0f * B - 6.0f * C) * v3 +
                        (-18.0f + 12.0f * B + 6.0f * C) * v2 +
                        (6.0f - 2.0f * B));
            }
            else
            {
                return (1.0f / 6.0f) *
                    ((-B - 6.0f * C) * v3 + (6.0f * B + 30.0f * C) * v2 +
                        (-12.0f * B - 48.0f * C) * v + (8.0f * B + 24.0f * C));
            }
        };
    ...
    {
        // Shio: Firefly Clamping & NaN prevention (Essential for BDPT to
        // prevent massive variance spikes)
        Color3f L = L_in;
        if(std::isnan(L.x) || std::isnan(L.y) || std::isnan(L.z)) return;

        // ONLY clamp indirect light (bounces).
        // Direct light handles primary rays (looking directly at the sun:
        // 2000.0f) If we clamp direct light, the sun turns dark grey or black!
        if(!is_direct)
        {
        ...
        int x1 =
            std::min(width - 1, (int)std::floor(px - 0.5f + filter_radius));
        int y0 = std::max(0, (int)std::ceil(py - 0.5f - filter_radius));
        int y1 =
            std::min(height - 1, (int)std::floor(py - 0.5f + filter_radius));
        ...
        auto atomic_add = [](std::atomic<double> & target, double val) {
            ...
            while(!target.compare_exchange_weak(
                old, old + val, std::memory_order_relaxed))
                ;
        };
        ...
        for(int y = y0; y <= y1; ++y)
        {
            for(int x = x0; x <= x1; ++x)
            {
                ...
                float weight = evaluate_filter(dx, dy) *
                    sample_weight_multiplier; // Shio: Energy conservation for
                                              // sparse rendering
                if(weight != 0.0f)
                {
                    int idx = y * width + x;
                    if(is_direct)
                    {
                        atomic_add(pixels[idx].direct_r,
                            static_cast<double>(L.x * weight));
                        atomic_add(pixels[idx].direct_g,
                            static_cast<double>(L.y * weight));
                        atomic_add(pixels[idx].direct_b,
                            static_cast<double>(L.z * weight));
                        atomic_add(
                            pixels[idx].direct_w, static_cast<double>(weight));
                    }
                    else
                    {
                        atomic_add(pixels[idx].indirect_r,
                            static_cast<double>(L.x * weight));
                        atomic_add(pixels[idx].indirect_g,
                            static_cast<double>(L.y * weight));
                        atomic_add(pixels[idx].indirect_b,
                            static_cast<double>(L.z * weight));
                        atomic_add(pixels[idx].indirect_w,
    ...
    void apply_ema_decay(double decay_factor, double expired_decay = 0.5)
    {
        ...
        // 'decay_factor' dictates how much history we keep. 1.0 = keep all
        // (infinite accumulation). 0.85 = typical standing decay (smooths
        // noise, keeps shadows responsive).
        for(auto & p : pixels)
        {
            auto atomic_scale = [](std::atomic<double> & target,
                                    double               factor) {
                double old = target.load(std::memory_order_relaxed);
                while(!target.compare_exchange_weak(
                    old, old * factor, std::memory_order_relaxed))
                    ;
            };

            // Shio: To prevent the "strobe blindness" from sparse dynamic
            // rendering when moving, we apply the explicit decay_factor to BOTH
            // direct and indirect evenly!
        ...
        // leaving only the crisp new samples behind without ever dipping to
        // zero-energy black!
...
struct ServiceScene
{
    ...
    // Shio: Rebalance and optimize the BVH using a Surface Area Heuristic (SAH)
    // build. This optimization method can be called dynamically at runtime if
    // new objects are added or if existing objects have moved significantly to
    // rebalance the tree.
    void optimize_bvh()
    {
        bvh_prims.clear();
        ...
        {
            AABB b;
            b.expand(spheres[i].center -
                Vector3f {
                    spheres[i].radius, spheres[i].radius, spheres[i].radius });
            b.expand(spheres[i].center +
                Vector3f {
                    spheres[i].radius, spheres[i].radius, spheres[i].radius });
    ...
    void subdivide_bvh_sah(int node_idx, int first, int count)
    {
        ...
        // Shio: Surface Area Heuristic (SAH). Instead of picking the median
        // primitive, we evaluate the probability of a random ray hitting the
        // left vs right child nodes. We approximate the continuous space by
        // slicing the bounding box into 8 discrete bins.
        for(int axis = 0; axis < 3; ++axis)
        {
            ...
            for(int i = 0; i < count; ++i)
            {
                float c       = bvh_prims[first + i].bounds.center()[axis];
                int   bin_idx = std::min(
                    BINS - 1, std::max(0, (int)((c - min_centroid) * scale)));
                if(bin_idx == BINS)
                    bin_idx =
                        BINS - 1; // Safeguard against floating point rounding
            ...
            for(int i = BINS - 1; i > 0; --i)
            {
                ...
                float cost = left_count[i - 1] * left_area[i - 1] +
                    sum_count * right_box.surface_area();
                if(cost < best_cost)
                {
                    best_cost = cost;
                    best_axis = axis;
                    best_split_pos =
                        min_centroid + i * (max_centroid - min_centroid) / BINS;
                }
            }
        }
        ...
        subdivide_bvh_sah(
            right_child_idx, first + left_count, count - left_count);
    }

    Color3f evaluate_emission(
        const Material & mat, const Vector3f & p, const Vector3f & center) const
    {
        if(!mat.is_moon) return mat.emission;

        // Procedural craters/surface noise
        Vector3f local = (p - center).normalize();
        float    n     = std::sin(local.x * 30.0f) * std::cos(local.y * 30.0f) *
            std::sin(local.z * 30.0f);
        n += 0.5f * std::sin(local.x * 60.0f + local.y * 60.0f);
        float noise = (n + 1.5f) * 0.4f;
        noise       = std::clamp(
            noise, 0.4f, 1.0f); // Prevents the moon from having jet-black voids
    ...
    Color3f evaluate_sky(const Vector3f & view_dir) const
    {
        ...
        auto calc_sky_for_light = [&](const Vector3f & l_dir,
            ...
            Vector3f sky = { (scatter.x / beta_sum.x) *
                    (1.0f - scatter_attenuation.x),
            ...
            Vector3f l_tau         = { beta_sum.x * opt_depth_s * 5.0f,
                beta_sum.y * opt_depth_s * 5.0f,
                beta_sum.z * opt_depth_s * 5.0f };
            Vector3f l_attenuation = {
                std::exp(-l_tau.x), std::exp(-l_tau.y), std::exp(-l_tau.z)
            };

            return { sky.x * l_attenuation.x * intensity.x,
        ...
        Color3f moon_sky = calc_sky_for_light(
            moon_dir, { 0.2f, 0.24f, 0.3f }); // Much dimmer moonlight
        ...
        // as it sets, it *must* undergo the same transmission attenuation as
        // the scattered sky!
        ...
        if(cos_theta_sun > std::cos(sun_angular_radius) && sun_dir.y > 0.0f)
        {
            ...
            Vector3f s_tau           = { beta_sum.x * (1.0f / s_y) * 5.0f,
                beta_sum.y * (1.0f / s_y) * 5.0f,
                beta_sum.z * (1.0f / s_y) * 5.0f };
            Vector3f sun_attenuation = {
                std::exp(-s_tau.x), std::exp(-s_tau.y), std::exp(-s_tau.z)
            };

            sun_sky += Color3f { 2000.0f * sun_attenuation.x,
        ...
        if(cos_theta_moon > std::cos(moon_angular_radius) && moon_dir.y > 0.0f)
        {
            ...
            // The moon is in a different position than the sun, so it has its
            // own unique atmospheric thickness
            float    opt_depth_m   = 1.0f / std::max(0.001f, moon_dir.y);
            Vector3f m_tau         = { beta_sum.x * opt_depth_m * 5.0f,
                beta_sum.y * opt_depth_m * 5.0f,
                beta_sum.z * opt_depth_m * 5.0f };
            Vector3f m_attenuation = {
                std::exp(-m_tau.x), std::exp(-m_tau.y), std::exp(-m_tau.z)
            };

            moon_sky += Color3f { 15.0f * m_attenuation.x,
        ...
        const Vector3f & o, const Vector3f & d, float t_max) const
    {
        ...
        Vector3f inv_d = { d.x == 0.0f ? 1e30f : 1.0f / d.x,
        ...
        while(stack_ptr > 0)
        {
            ...
            // Shio: Fast AABB intersection test. If the ray completely misses
            // this node's bounding box, we prune the entire sub-tree,
            // accelerating traversal from O(n) to O(log n).
                ...
                for(int i = 0; i < node.prim_count; ++i)
                {
                    ...
                    if(prim.type == 0)
                    {
                        ...
                        if(auto t = s.intersect(o, d, 0.001f, rec.t))
                        {
                            rec.t      = *t;
                            rec.point  = o + d * rec.t;
                            rec.normal = (rec.point - s.center)
                                             .normalize(); // STRICTLY OUTWARD
                    ...
                    else
                    {
                        ...
                        if(auto t = b.intersect(o, d, 0.001f, rec.t))
                        {
                            ...
                            rec.point = o + d * rec.t;
                            rec.normal =
                                b.get_normal(rec.point); // STRICTLY OUTWARD
    ...
    bool sample_light(SamplerPCG32 & rng,
        Vector3f &                   p,
        Normal3f &                   n,
        Color3f &                    emission,
        float &                      pdf) const
    {
        ...
        int light_idx =
            std::min((int)(rng.next_float() * light_count), light_count - 1);
        ...
        for(const auto & s : spheres)
        {
            if(materials[s.material_index].type == MaterialType::Light)
            {
                if(current_idx++ == light_idx)
                {
                    ...
                    pdf /=
                        light_count; // Scale pdf by uniform choice probability

                    Color3f base_emission = evaluate_emission(
                        materials[s.material_index], p, s.center);

                    // Atmosphere attenuation for BDPT light path origins
                    // Shio: p is the position on the sun/moon. The direction
                    // FROM the origin TO the light is simply p.normalize()
                    Vector3f dir_to_light = p.normalize();
                    float    l_y          = std::max(0.001f, dir_to_light.y);
                    float    opt_depth_l  = 1.0f / l_y;
                    Vector3f tau = { (0.0038f + 0.0210f) * opt_depth_l * 5.0f,
                    ...
                    emission = { base_emission.x * attenuation.x,
                        base_emission.y * attenuation.y,
                        base_emission.z * attenuation.z };
                    return true;
                }
            }
        }
            ...
            if(materials[b.material_index].type == MaterialType::Light)
            {
                if(current_idx++ == light_idx)
                {
                    ...
                    emission        = evaluate_emission(
                        materials[b.material_index], p, center);
                    return true;
                }
            }
        }
...
inline float compute_mis_weight(
    const PathVertex * path, int k, int s, float light_area)
{
    float sum_W2 = 0.0f;
    float W2_s   = 0.0f;
...
{
    using WriteComponent = Usagi::ComponentList<ComponentRay,
        ComponentPathState,
        ComponentCameraPath,
        ComponentLightPath>;
    ...
    void update(auto && entities, auto && services)
    {
        ...
        int    actual_count =
            std::min((int)total_pixels, std::max(1'000, state_svc.ray_budget));
        ...
        if(state_svc.pass == 0)
        {
            ...
            // This natively prevents "strobe strips" and creates a uniform
            // white-noise visual resolution field when rendering sparsely
            // during fast movement!
            state_svc.active_pixels.resize(actual_count);
            for(int i = 0; i < actual_count; ++i)
            {
                state_svc.active_pixels[i] =
                    state_svc.next_u32() % total_pixels;
            }
        }
        ...
        size_t chunk_size = std::max<size_t>(
            1, count / (std::thread::hardware_concurrency() * 4));
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    if(state.rng.inc == 0)
                    {
                        // Shio: O(1) temporal seeding instead of looping. The
                        // frame count guarantees a unique temporal stream of
                        // stochastic samples over time.
                        state.rng.seed(pixel.y * canvas.width + pixel.x + 1,
                            1'337 + canvas.frame_count.load());
                        // Generate a random pixel-specific offset for the
                        // Halton sequence to prevent structural grid patterns
                        // across neighboring pixels
                        state.sample_index = state.rng.next_u32();
                    }

                    // Advance sample index to guarantee a unique sequence point
                    // each frame
                    uint32_t current_sample =
                        state.sample_index + canvas.frame_count.load();
                    ...
                    if(is_cam)
                    {
                        // Jitter for anti-aliasing (Base-2 / Base-3 Radical
                        // Inverse)
                        ...
                        ray.origin    = cam_pos;
                        ray.direction = (cam_fwd + cam_right * px + cam_up * py)
                                            .normalize();
                        ray.t_max = 1000.0f;
                    ...
                    else
                    {
                        ...
                        if(scene.sample_light(state.rng, p, n, e, pdf))
                        {
                            ...
                            ONB onb;
                            onb.build_from_w(n);
                            Vector3f dir = onb.local(
                                cosine_sample_hemisphere(state.rng.next_float(),
                                    state.rng.next_float()));

                            // First light vertex is the point on the light
                            // source. Setting albedo to PI ensures the 1/PI
                            // lambertian factor correctly cancels when
                            // connecting directly.
                            lpaths[entity_idx].vertices[0] = {
                                p, n, state.throughput, { PI, PI, PI }, false
                            };
...
/*
 * Shio: Intersects active rays with the scene and routes them to material
 * queues. Decoupled from material evaluation to demonstrate data-oriented
 * processing.
 */
struct SystemIntersectRays
{
    ...
    using WriteService =
        Usagi::ComponentList<ServiceRayQueue, ServiceScheduler, ServiceFilm>;
            ...
            count, chunk_size, [&](size_t start, size_t end) {
                ...
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    if(opt_hit)
                    {
                        ...
                        // The 'normal' field coming from primitives is strictly
                        // OUTWARD-pointing.
                        hit_c.hit.set_face_normal(
                            ray.direction, hit_c.hit.normal);

                        const auto & mat =
                            scene.materials[opt_hit->material_index];

                        if(state.depth >= MAX_PATH_DEPTH)
                        {
                            state.active = false;
                            loc_light.push_back(
                                id); // Send to light to pick up emission, but
                                     // it won't bounce
                        }
                    ...
                    else
                    {
                        ...
                        { // Camera paths
                            ...
                            // If this is the primary camera ray, it's direct
                            // (cleared every frame). If it bounced, it's
                            // indirect (EMA decayed).
                            film.add_sample(state.sample_x,
                                state.sample_y,
                                total_sky_L,
                                state.depth == 0);
                        }
                ...
                ray_queue.q_lambert.insert(ray_queue.q_lambert.end(),
                    loc_lambert.begin(),
                    loc_lambert.end());
                ray_queue.q_metal.insert(ray_queue.q_metal.end(),
                    loc_metal.begin(),
                    loc_metal.end());
                ray_queue.q_light.insert(ray_queue.q_light.end(),
                    loc_light.begin(),
                    loc_light.end());
                ray_queue.q_translucent.insert(ray_queue.q_translucent.end(),
                    loc_translucent.begin(),
                    loc_translucent.end());
            });
    }
};
...
/*
 * Shio: Evaluates Translucent materials using Schlick's approximation for
        ...
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                ...
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    {
                        cpaths[id].vertices[cpaths[id].count++] = { hit.point,
                            hit.normal,
                            state.throughput,
                            mat.albedo,
                            true };
                    ...
                    {
                        lpaths[id].vertices[lpaths[id].count++] = { hit.point,
                            hit.normal,
                            state.throughput,
                            mat.albedo,
                            true };
                    ...
                    if(!hit.front_face)
                    {
                        // Shio: Beer's Law (Volumetric Absorption for
                        // Jelly/SSS)
                        float    dist       = hit.t;
                        Vector3f absorption = { 1.0f - mat.albedo.x,
                            1.0f - mat.albedo.y,
                            1.0f - mat.albedo.z };
                        state.throughput.x *=
                            std::exp(-absorption.x * dist * mat.density);
                        state.throughput.y *=
                            std::exp(-absorption.y * dist * mat.density);
                        state.throughput.z *=
                            std::exp(-absorption.z * dist * mat.density);
                    }

                    // Determine if we are entering or exiting the medium using
                    // explicit front_face flag hit.normal is guaranteed by
                    // SystemIntersectRays to point AGAINST ray.direction
                    float eta = hit.front_face ? (1.0f / mat.ior) : mat.ior;
                    float cos_theta_i =
                        std::min(hit.normal.dot(ray.direction * -1.0f), 1.0f);
                    ...
                    float sin_theta_t_sq =
                        eta * eta * (1.0f - cos_theta_i * cos_theta_i);
                    ...
                    {
                        // Reflect (hit.normal is against ray, so standard
                        // reflection works)
                        scatter_dir =
                            ray.direction + hit.normal * 2.0f * cos_theta_i;
                    }
                    else
                    {
                        // Refract
                        float cos_theta_t = std::sqrt(1.0f - sin_theta_t_sq);
                        // Since hit.normal is against ray, inward normal is
                        // -hit.normal
                        scatter_dir       = ray.direction * eta +
                            hit.normal * (eta * cos_theta_i - cos_theta_t);
                    }

                    if(mat.roughness > 0.0f)
                    {
                        scatter_dir = scatter_dir +
                            random_in_unit_sphere(state.rng) * mat.roughness;
                    }

                    scatter_dir = scatter_dir.normalize();

                    // Shio: Bias origin OUTWARD from the surface if reflecting,
                    // INWARD if refracting. Since hit.normal points against the
                    // incoming ray (OUTWARD from the surface we hit): If
                    // reflecting, scatter_dir is on the same side as
                    // hit.normal. dot > 0. If refracting, scatter_dir is on the
                    // opposite side. dot < 0.
                    float bias_dir =
                        scatter_dir.dot(hit.normal) > 0.0f ? 1.0f : -1.0f;
                ...
                std::lock_guard<std::mutex> lock(merge_mutex);
                ray_queue.next_rays.insert(ray_queue.next_rays.end(),
                    loc_next.begin(),
                    loc_next.end());
            });
    }
        ...
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                ...
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    if(is_cam && cpaths[id].count < MAX_PATH_DEPTH)
                    {
                        // Shio: Store the BRDF value (albedo / PI) in the
                        // vertex for the BDPT connector to evaluate!
                        cpaths[id].vertices[cpaths[id].count++] = { hit.point,
                            hit.normal,
                            state.throughput,
                            mat.albedo * (1.0f / PI),
                    ...
                    // Shio: Independent PCG32 samples correctly map to uniform
                    // points in the 2D domain without Weyl sequence correlation
                    // patterns.
                    float u1 = state.rng.next_float();
                    float u2 = state.rng.next_float();

                    Vector3f scatter_dir =
                        onb.local(cosine_sample_hemisphere(u1, u2));

                    ray.origin    = hit.point + hit.normal * 0.001f;
                    ray.direction = scatter_dir.normalize();
                    ray.t_max     = 1000.0f;

                    // Because PDF = cos(theta) / PI and BRDF = albedo / PI,
                    // they cancel out exactly ONLY for the indirect recursive
                    // throughput!
                    state.throughput = state.throughput * mat.albedo;
                    state.depth++;
        ...
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                ...
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    Vector3f reflected = ray.direction -
                        hit.normal * 2.0f * ray.direction.dot(hit.normal);
                    ...
                    if(target.dot(hit.normal) > 0.0f)
                    {
                        ...
                        if(state.depth <
                            (is_cam ? MAX_PATH_DEPTH : MAX_PATH_DEPTH))
                        {
                            loc_next.push_back(id);
                        }
                        else
...
struct SystemEvaluateLight
{
    using WriteComponent =
        Usagi::ComponentList<ComponentPathState, ComponentCameraPath>;
    using ReadComponent = Usagi::ComponentList<ComponentRayHit>;
    using ReadService = Usagi::ComponentList<ServiceScene, ServiceRenderState>;
    using WriteService =
        Usagi::ComponentList<ServiceRayQueue, ServiceScheduler, ServiceFilm>;
        ...
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    if(state_svc.pass == 1)
                    { // Removed last_bounce_specular check to cover diffuse
                      // direct hits
                        auto & cpath = cpaths[id];

                        Color3f raw_emission = scene.evaluate_emission(
                            mat, hit.point, hit.obj_center);

                        // Compute atmospheric attenuation from the viewer to
                        // the celestial body Shio: hit.point is on the sun.
                        // Direction TO the light is hit.point.normalize()
                        Vector3f dir_to_light = hit.point.normalize();
                        float    l_y         = std::max(0.001f, dir_to_light.y);
                        float    opt_depth_l = 1.0f / l_y;
                        Vector3f tau = { (0.0038f + 0.0210f) * opt_depth_l *
                                5.0f,
                            (0.0135f + 0.0210f) * opt_depth_l * 5.0f,
                            (0.0331f + 0.0210f) * opt_depth_l * 5.0f };
                        Vector3f attenuation = {
                        ...
                        Color3f emission = { raw_emission.x * attenuation.x,
                            raw_emission.y * attenuation.y,
                            raw_emission.z * attenuation.z };

                        if(state.depth == 0)
                        {
                            // Pure direct hits bypass MIS and go straight to
                            // the direct buffer!
                            film.add_sample(state.sample_x,
                                state.sample_y,
                        ...
                        else
                        {
                            ...
                            // Shio: Light source vertices MUST have an
                            // effective BRDF of 1.0 (so albedo = PI to cancel
                            // the /PI in ConnectPaths).
                            path[0] = { hit.point,
                                hit.normal,
                                { 1, 1, 1 },
                                { PI, PI, PI },
                            ...
                            {
                                path[1 + j] =
                                    cpath.vertices[cpath.count - 1 - j];
                            }

                            float light_area = scene.get_light_area();
                            float mis_weight = compute_mis_weight(
                                path, cpath.count + 1, 0, light_area);

                            cpath.direct_emission =
                                state.throughput * emission * mis_weight;
                        }
                    }
                    state.active = false;
                }
...
{
    using ReadComponent = Usagi::ComponentList<ComponentCameraPath,
        ComponentLightPath,
        ComponentPathState>;
    using WriteService  = Usagi::ComponentList<ServiceFilm>;
    using ReadService   = Usagi::
        ComponentList<ServiceRenderState, ServiceScheduler, ServiceScene>;
        ...
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    // Explicit direct hit (bounced via material evaluations
                    // hitting light source directly).
                    if(cpath.count > 0 &&
                        (cpath.direct_emission.x > 0.0f ||
                            cpath.direct_emission.y > 0.0f ||
                            cpath.direct_emission.z > 0.0f))
                    {
                    ...
                    for(int t = 1; t <= cpath.count; ++t)
                    {
                        ...
                        for(int s = 1; s <= lpath.count; ++s)
                        {
                            ...
                            {
                                if(!scene.intersect(
                                       cv.p + cv.n * 0.001f, L, dist - 0.002f))
                                {
                                    float   G = (ndotl_c * ndotl_l) / dist2;
                                    Color3f brdf_c = cv.albedo;
                                    Color3f brdf_l = lv.albedo;
                                    Color3f contrib =
                                        cv.beta * brdf_c * G * brdf_l * lv.beta;

                                    PathVertex path[MAX_PATH_DEPTH * 2];
                                    for(int j = 0; j < s; ++j)
                                        path[j] = lpath.vertices[j];
                                    for(int j = 0; j < t; ++j)
                                        path[s + j] = cpath.vertices[t - 1 - j];

                                    float mis_weight = compute_mis_weight(
                                        path, s + t, s, light_area);

                                    // All connection paths are treated as
                                    // indirect (EMA decayed) since they
                                    // represent scattered light. Only 0-bounce
                                    // direct eye hits go to direct_L.
                    ...
                    if(direct_L.x > 0 || direct_L.y > 0 || direct_L.z > 0)
                        film.add_sample(
                            state.sample_x, state.sample_y, direct_L, true);
                    if(indirect_L.x > 0 || indirect_L.y > 0 || indirect_L.z > 0)
                        film.add_sample(
                            state.sample_x, state.sample_y, indirect_L, false);
                }
            });
    }
};
...
struct SystemPathTracingCoordinator
{
    ...
    using WriteService = Usagi::ComponentList<ServiceRayQueue,
        ServiceScheduler,
        ServiceRenderState,
        ServiceCamera,
        ServiceTime>;
    ...
    void update(auto && entities, auto && services)
    {
        ...
        if(state_svc.pass == 0)
        {
            ...
            // Track if time is actively advancing (e.g., normal playback or
            // forced fast-forward)
            ...
            float fraction =
                (float)state_svc.ray_budget / (canvas.width * canvas.height);
            ...
            // A tunable multiplier applies a 25% faster decay rate (0.9 ^ 1.25)
            // so it aggressively fades out.
            double expired_decay = std::clamp<double>(
                std::pow(1.0f - fraction, film.expired_decay_scalar),
                0.0,
                0.99);

            if(camera_moved || time_moved)
            {
                // Shio: Keep a trace going so user isn't blind during heavy
                // movement
                film.apply_ema_decay(
                    0.6, 0.0); // Wipe expired immediately when moving
            ...
            else
            {
                if(was_moving)
                {
                    ...
                    // 1. Shove the entire dirty motion-blurred frame into the
                    // rapidly decaying `expired` buffer.
                    // 2. Clear the active buffer to 0.
                    // 3. From now on, the clean active buffer builds up pure
                    // sparse samples and replaces the ghost perfectly!
                ...
                {
                    // Infinite accumulation (decay = 1.0) merges all frames
                    // perfectly when paused and completely still
                    film.apply_ema_decay(1.0, expired_decay);
                }
                else
                {
                    film.apply_ema_decay(0.85,
                        expired_decay); // Normal standing decay (0.85). Smooths
                                        // out the 4-SPP noise while keeping
                                        // shadows responsive!
                }
            }
            ...
            scene.sun_dir =
                Vector3f { 0.0f, std::cos(time), std::sin(time) }.normalize();
            // Moon orbits offset in phase and tilted slightly in X so they
            // never collide
            scene.moon_dir =
                Vector3f { 0.3f, std::cos(time + PI), std::sin(time + PI) }
                    .normalize();
            ...
            if(scene.spheres.size() >= 2)
            {
                ...
                // The physical scattering equations in evaluate_sky and
                // evaluate_light will naturally scatter blue light away,
                // turning the *remaining* transmitted beam orange without
                // artificially hacking the source color.
                if(scene.sun_dir.y > 0.0f)
                {
                    scene.materials[scene.spheres[0].material_index]
                        .emission = { 2000.0f, 1000.0f, 500.0f };
                }
                else
                {
                    scene.materials[scene.spheres[0].material_index]
                ...
                // Base moon color (pale blue/white). Moon is very dim compared
                // to the sun.
                if(scene.moon_dir.y > 0.0f)
                {
                    scene.materials[scene.spheres[1].material_index]
                        .emission = { 15.0f, 18.0f, 22.0f };
                }
                else
                {
                    scene.materials[scene.spheres[1].material_index]
            ...
            scene.optimize_bvh(); // Shio: Rebalance and optimize BVH as dynamic
                                  // objects have moved!
...
struct SystemRenderGDICanvas
{
    ...
    void update(auto && entities, auto && services)
    {
        ...
        // Shio: Fast 3x3 Edge-Avoiding Bilateral filter pass using luminance
        // differentials to smooth noise without destroying structural detail.
        size_t chunk_size = std::max<size_t>(
            1, count / (std::thread::hardware_concurrency() * 4));
        scheduler.host->parallel_for(
            count, chunk_size, [&](size_t start, size_t end) {
                for(size_t i = start; i < end; ++i)
                {
                    ...
                    double d_w = film.pixels[idx].direct_w.load(
                                     std::memory_order_relaxed) +
                        film.expired_pixels[idx].direct_w.load(
                            std::memory_order_relaxed);
                    double i_w = film.pixels[idx].indirect_w.load(
                                     std::memory_order_relaxed) +
                        film.expired_pixels[idx].indirect_w.load(
                            std::memory_order_relaxed);
                    ...
                    if(total_w > 0.0)
                    {
                        ...
                        double active_sum_r = film.pixels[idx].direct_r.load(
                                                  std::memory_order_relaxed) +
                            film.pixels[idx].indirect_r.load(
                                std::memory_order_relaxed);
                        double active_sum_g = film.pixels[idx].direct_g.load(
                                                  std::memory_order_relaxed) +
                            film.pixels[idx].indirect_g.load(
                                std::memory_order_relaxed);
                        double active_sum_b = film.pixels[idx].direct_b.load(
                                                  std::memory_order_relaxed) +
                            film.pixels[idx].indirect_b.load(
                                std::memory_order_relaxed);

                        double exp_sum_r =
                            film.expired_pixels[idx].direct_r.load(
                                std::memory_order_relaxed) +
                            film.expired_pixels[idx].indirect_r.load(
                                std::memory_order_relaxed);
                        double exp_sum_g =
                            film.expired_pixels[idx].direct_g.load(
                                std::memory_order_relaxed) +
                            film.expired_pixels[idx].indirect_g.load(
                                std::memory_order_relaxed);
                        double exp_sum_b =
                            film.expired_pixels[idx].direct_b.load(
                                std::memory_order_relaxed) +
                            film.expired_pixels[idx].indirect_b.load(
                                std::memory_order_relaxed);

                        color_center.x = static_cast<float>(
                            (active_sum_r + exp_sum_r) * inv);
                        color_center.y = static_cast<float>(
                            (active_sum_g + exp_sum_g) * inv);
                        color_center.z = static_cast<float>(
                            (active_sum_b + exp_sum_b) * inv);
                    }

                    float lum_center = color_center.x * 0.2126f +
                        color_center.y * 0.7152f + color_center.z * 0.0722f;
                    ...
                    for(int dy = -1; dy <= 1; ++dy)
                    {
                        ...
                        {
                            int nx =
                                std::clamp(pixel.x + dx, 0, canvas.width - 1);
                            int ny =
                                std::clamp(pixel.y + dy, 0, canvas.height - 1);
                            int nidx = ny * canvas.width + nx;

                            Color3f neighbor = { 0, 0, 0 };

                            double n_d_w = film.pixels[nidx].direct_w.load(
                                               std::memory_order_relaxed) +
                                film.expired_pixels[nidx].direct_w.load(
                                    std::memory_order_relaxed);
                            double n_i_w = film.pixels[nidx].indirect_w.load(
                                               std::memory_order_relaxed) +
                                film.expired_pixels[nidx].indirect_w.load(
                                    std::memory_order_relaxed);
                            ...
                            if(n_total_w > 0.0)
                            {
                                ...
                                double n_active_sum_r =
                                    film.pixels[nidx].direct_r.load(
                                        std::memory_order_relaxed) +
                                    film.pixels[nidx].indirect_r.load(
                                        std::memory_order_relaxed);
                                double n_active_sum_g =
                                    film.pixels[nidx].direct_g.load(
                                        std::memory_order_relaxed) +
                                    film.pixels[nidx].indirect_g.load(
                                        std::memory_order_relaxed);
                                double n_active_sum_b =
                                    film.pixels[nidx].direct_b.load(
                                        std::memory_order_relaxed) +
                                    film.pixels[nidx].indirect_b.load(
                                        std::memory_order_relaxed);

                                double n_exp_sum_r =
                                    film.expired_pixels[nidx].direct_r.load(
                                        std::memory_order_relaxed) +
                                    film.expired_pixels[nidx].indirect_r.load(
                                        std::memory_order_relaxed);
                                double n_exp_sum_g =
                                    film.expired_pixels[nidx].direct_g.load(
                                        std::memory_order_relaxed) +
                                    film.expired_pixels[nidx].indirect_g.load(
                                        std::memory_order_relaxed);
                                double n_exp_sum_b =
                                    film.expired_pixels[nidx].direct_b.load(
                                        std::memory_order_relaxed) +
                                    film.expired_pixels[nidx].indirect_b.load(
                                        std::memory_order_relaxed);

                                neighbor.x = static_cast<float>(
                                    (n_active_sum_r + n_exp_sum_r) * inv);
                                neighbor.y = static_cast<float>(
                                    (n_active_sum_g + n_exp_sum_g) * inv);
                                neighbor.z = static_cast<float>(
                                    (n_active_sum_b + n_exp_sum_b) * inv);
                            }

                            float lum_neighbor = neighbor.x * 0.2126f +
                                neighbor.y * 0.7152f + neighbor.z * 0.0722f;

                            // Edge-stopping function based on luminance
                            // difference
                            float l_diff = lum_neighbor - lum_center;
                            float w_l    = std::exp(-(l_diff * l_diff) /
                                0.1f); // Range sensitivity tuning
                    ...
                    Color3f color = w_blur_sum > 0.0f
                        ? c_blur * (1.0f / w_blur_sum)
                        : color_center;
                    ...
                    // This gracefully handles extremely bright 2000.0+ HDR
                    // pixels (Sun Core)
                    ...
                    // Exposure adjustment. Lower exposure maps the intense
                    // sun/sky into SDR range without blowout
                    float exposure = 0.1f;
                    color.x *= exposure;
                    color.y *= exposure;
                    color.z *= exposure;
