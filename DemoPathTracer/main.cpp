...
struct SystemPresentToGDI
{
    void update(auto && entities, auto && services)
    {
        ...
        SetDIBitsToDevice(hdc, 0, 0, canvas.width, canvas.height, 0, 0, 0, canvas.height, canvas.pixel_buffer, &bmi, DIB_RGB_COLORS);
...
void SetupCornellBox(RT::ServiceScene & scene)
{
    ...
    // Materials
    // 0: White
    scene.materials.push_back({ MaterialType::Lambert, { 0.73f, 0.73f, 0.73f }, { 0, 0, 0 }, 0 });
    // 1: Red
    scene.materials.push_back({ MaterialType::Lambert, { 0.65f, 0.05f, 0.05f }, { 0, 0, 0 }, 0 });
    // 2: Green
    scene.materials.push_back({ MaterialType::Lambert, { 0.12f, 0.45f, 0.15f }, { 0, 0, 0 }, 0 });
    // 3: Light
    scene.materials.push_back({ MaterialType::Light,   { 1.0f, 1.0f, 1.0f },    { 15.0f, 15.0f, 15.0f }, 0 });
    ...
    // Boxes inside
    // Short box
    scene.boxes.push_back({ { -3.0f, -s, 2.0f }, { 0.0f, -s + 3.0f, 5.0f }, 0 });
    // Tall box
    scene.boxes.push_back({ { 2.0f, -s, -3.0f }, { 5.0f, -s + 6.0f, 0.0f }, 0 });
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    g_hWnd = CreateWindowEx(0, wc.lpszClassName, L"Usagi Engine - Cornell Box Path Tracer", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, wr.right - wr.left, wr.bottom - wr.top, nullptr, nullptr, hInstance, nullptr);
    ...
    Usagi::ComponentGroup<RT::ComponentPixel, RT::ComponentRay, RT::ComponentPathState> primary_group(primary_heap, PIXEL_COUNT);

    // Services
    RT::ServiceGDICanvasProvider gdi_canvas;
    gdi_canvas.width = WIDTH;
    gdi_canvas.height = HEIGHT;
    gdi_canvas.pixel_buffer = primary_heap.allocate_pod<uint32_t>(PIXEL_COUNT).resolve(primary_heap.get_base());
    ...
    while(true)
    {
        ...
        // Shio: Cyclic Task Graph Execution
        // 1. Generate Primary Rays
        executive.dispatch(sys_gen_rays, primary_group, services);

        // 2. Iterative Bounces (Queue processing simulation)
        // We bounce 5 times per frame.
        for(int i = 0; i < 5; ++i)
        {
        ...
        executive.dispatch(sys_present, primary_group, services);
    }
